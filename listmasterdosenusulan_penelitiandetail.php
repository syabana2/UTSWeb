...
if(isset($_GET['NIDN'])){echo "<a href=insertmasterdosenusulan_penelitiandetail.php?NIDN=". mysqli_real_escape_string($con, $_GET['NIDN']) ."><button type='button' class='btn btn-light'><font face=Verdana size=1><i class='fa fa-plus'></i>&nbsp;Insert</font></button></a><br><br>";}
...
if(isset($_GET['NIDN'])){$result = mysqli_query($con, "SELECT * FROM usulan_penelitian where usulan_penelitian.Pembimbing= '".mysqli_real_escape_string($con, $_GET['NIDN'])."' LIMIT $posisi,$batas");}
echo "<div class='table-responsive'> "; 
echo "<table class='table table-striped'>"; 
$firstColumn = 1;
$warna = 0;
...
$firstColumn = 0;
  ...
  if(isset($_GET['NIDN'])){echo "<td><a class=linklist href=viewmasterdosenusulan_penelitiandetail.php?id=".$row['id']."&NIDN=". mysqli_real_escape_string($con, $_GET['NIDN']) ."><button type='button' class='btn btn-warning'><font face=Verdana size=1><i class='fa fa-check'></i></font></button></a></td>";}
  if(isset($_GET['NIDN'])){echo "<td><a class=linklist href=editmasterdosenusulan_penelitiandetail.php?id=".$row['id']."&NIDN=". mysqli_real_escape_string($con, $_GET['NIDN']) ."><button type='button' class='btn btn-primary'><font face=Verdana size=1><i class='fa fa-edit'></i></font></button></a></td>";}
  if(isset($_GET['NIDN'])){echo "<td><a class=linklist href=deletemasterdosenusulan_penelitiandetail.php?id=".$row['id']."&NIDN=". mysqli_real_escape_string($con, $_GET['NIDN']) ." onclick=\"return confirm('Are you sure you want to delete this data?')\"><button type='button' class='btn btn-danger'><font face=Verdana size=1><i class='fa fa-Trash'></i></font></button></a></td>";}
...
if(isset($_GET['NIDN'])){$tampil2 = mysqli_query($con, "SELECT * FROM usulan_penelitian where usulan_penelitian.Pembimbing='".mysqli_real_escape_string($con, $_GET['NIDN'])."'");}
