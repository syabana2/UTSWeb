...
echo "<form action=editdosenexec.php method=post>";
$result = mysqli_query($con, "SELECT * FROM dosen where NIDN = '". mysqli_real_escape_string($con, $_GET['NIDN']) . "'");
while($row = mysqli_fetch_array($result))
{
echo "<tr><td colspan=2><input type=hidden name=pk value='" . $row['NIDN'] . "'></td></tr>";
...
echo "<tr><td bgcolor=CCCCCC><font face=Verdana color=black size=1>&nbsp;&nbsp;NIDN&nbsp;&nbsp;</font></td>";
echo "<td bgcolor=CCEEEE><input type=text  class='form-control' name='NIDN' value='". $row['NIDN'] ."' size = 100></td>";
